...
int main(int argc, char **argv) {
  ...
  RobotInfo robot1_info(nh, "Mir100", "567A359");
  ...
  robot1_info.updateIpAddress("169.254.5.180");
  robot1_info.updateFirmwareVersion("3.5.8");
  ...
  //   robot1_info.getFirmwareVersion().c_str());

  // Set up publishing rate
  ros::Rate loop_rate(1); // 1 Hz

  //   ROS_INFO("Starting to publish robot info on 'robot1_info' topic...");
  //   ROS_INFO("You can listen to the topic using: rostopic echo
  //   /robot1_info");
